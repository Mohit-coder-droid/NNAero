...
def softmax(
    *args: float | np.ndarray,
    ...
    out = max + np.log(
        sum([np.exp(np.maximum(array - max, -500)) for array in args])
    )
    out = out * softness
    return out
...
def softmax_scalefree(
    *args: float | np.ndarray,
    ...
    return softmax(*args, softness=relative_softness * np.linalg.norm(np.array(args)))
...
def softplus(
    ...
    if np.is_casadi_type(x, recursive=False):
        ...
        return 1 / beta * np.logaddexp(0, beta * x)
